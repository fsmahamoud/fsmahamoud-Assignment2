...
def rss_mem_of_pid(proc_id: int) -> int:
   ...
   try:
       # Construct the file path for the process status
       status_file = f"/proc/{proc_id}/status"
       # Open the status file
       with open(status_file, 'r') as file:
           # Read the lines
           for line in file:
               # Find the line containing RSS
               if line.startswith("VmRSS"):
                   # Get the RSS value
                   rss = int(line.split()[1])  # VmRSS is in kB
                   return rss
   except FileNotFoundError:
       print(f"Process {proc_id} not found.")
   except ValueError:
       print("Error parsing memory usage data.")    
